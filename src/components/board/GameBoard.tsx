...
export default function GameBoard({ gameState, lastEvent }: GameBoardProps) {
  ...
  useEffect(() => {
    if (lastEvent?.event === 'game:answerRevealed') {
      const { answerIndex } = lastEvent.payload as { answerIndex: number };
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', display: 'flex', flexDirection: 'column', p: 3 }}>
      {/* Question */}
      <Box sx={{ textAlign: 'center', mb: 3, minHeight: 80, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {phase === 'lobby' && (
          <Typography variant="h4" sx={{ color: 'primary.main' }}>MINECRAFT FAMILY FEUD</Typography>
        )}
        {phase === 'gameOver' && (
          <Typography variant="h4" sx={{ color: 'primary.main' }}>GAME OVER</Typography>
        )}
      ...
      {showBoard && (
        <Box sx={{ display: 'flex', gap: 2, flex: 1, mb: 3 }}>
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
            {leftColumn.map((a, i) => (
              <AnswerTile key={i} index={i} text={a.text} count={a.count} revealed={revealedAnswers.includes(i)} animating={animatingIndex === i} />
            ))}
          </Box>
          <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
            {rightColumn.map((a, i) => (
              <AnswerTile key={midpoint + i} index={midpoint + i} text={a.text} count={a.count} revealed={revealedAnswers.includes(midpoint + i)} animating={animatingIndex === midpoint + i} />
            ))}
          </Box>
        </Box>
      )}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', px: 2 }}>
        {teams.map((team) => (
          <Box key={team.name} sx={{ textAlign: 'center', bgcolor: 'background.paper', border: '3px solid', borderColor: 'divider', px: 4, py: 2 }}>
            <Typography variant="h6" sx={{ color: 'text.secondary', fontSize: '0.7rem', mb: 1 }}>{team.name}</Typography>
            <Typography variant="h4" sx={{ color: 'primary.main', fontSize: '1.5rem' }}>{scores[team.name] ?? 0}</Typography>
          </Box>
        ))}
      </Box>
    </Box>
